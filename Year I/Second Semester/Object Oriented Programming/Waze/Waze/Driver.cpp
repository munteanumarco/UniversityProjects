...
#include "Utils.h"
#include <sstream>
Driver::Driver(string name, string status, float latitude, float longitude, int score)
...
{
	stringstream out;
	out << this->name << ";" << this->status << ";" << this->latitude << ";" << this->longitude << ";" << this->score << "\n";
	return out.str();
}
...
{
	return (this->latitude == d.latitude) && (this->longitude == d.longitude) && (this->name == d.name);
}
