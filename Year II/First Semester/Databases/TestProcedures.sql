...
-- Views
-- view with a SELECT statment on one table
GO 
CREATE OR ALTER VIEW usersOlderThan25 AS
	SELECT U.uName, U.age
	FROM AppUser U
	WHERE U.age > 25
...
GO
CREATE OR ALTER VIEW workoutsAndTrainers AS
	SELECT T.tName, P.pName
	FROM Program P INNER JOIN Trainer T ON P.trainerId = T.trainerId

-- a view with a SELECT statement that has a GROUP BY clause and operates on at least 2 tables
GO
CREATE OR ALTER VIEW groupUsersByWorkouts AS
	SELECT U.userId, U.uName, COUNT(*) AS workouts
	FROM AppUser U
	INNER JOIN WorkoutLog W ON U.userId = W.userId
	GROUP BY U.userId, U.uName

GO

EXEC addToViews 'usersOlderThan25'
EXEC addToViews 'workoutsAndTrainers'
EXEC addToViews 'groupUsersByWorkouts'
