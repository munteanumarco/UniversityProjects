...
class GraphUI:
    ...
    def print_menu(self):
        print("0.Exit application\n"
              ...
              "22.Find the minimum cost walk between two vertices\n")
    ...
    def lowest_length_path(self):
        start_vertex = int(input("Please insert the first vertex:"))
        stop_vertex = int(input("Please insert the second vertex:"))
        path = self.__graph.lowest_length_path(start_vertex,stop_vertex)

        if path is None:
            print("No path between " + str(start_vertex) + " and " + str(stop_vertex))
        else:
            print(f'The shortest path between {start_vertex} and {stop_vertex} has the length {len(path)-1}: ',end='')
            for i in range(len(path)):
                print(str(path[i]) + " ",end='')
            print()

    def minimum_cost_walk(self):
        s = int(input("Please insert the start vertex:"))
        t = int(input("Please insert the end vertex:"))
        prev = [-1] * self.__graph.number_of_vertices
        try:
            cost = self.__graph.minimum_cost_walk(s, t, prev)
            if cost != 1<<27:
                print("Cost of lowest cost walk: " + str(cost))
                res = "Lowest cost walk: "
                stack = []
                while prev[t] != -1:
                    stack.append(t)
                    t = prev[t]
